...
> 🎓 **Interactive Learning Module:** [phase3_mlp_mnist.html](phase3_mlp_mnist.html) — 15 interactive slides covering: Architecture 784→128→64→10, Fixed vs Choice hyperparameters, Pick Batch/Epochs/LR (with embedded LR→Epochs simulator), **Accuracy formula** (correct/total), **Cross-Entropy Loss** (−log(p_correct) with visual extractor + good ranges), **R²/MAE/RMSE** regression metrics (formulas, examples, good ranges), **Precision/Recall/F1** (confusion matrix, cancer detector example, tradeoffs), ReLU ("kill negatives" + bouncer analogy), Why ReLU breaks linearity, Softmax (scores→percentages + election analogy), Full pipeline walkthrough with real numbers, Detective analogy, CUDA kernels + 30× vs PyTorch benchmark. Also: [🧮 LR→Epochs Estimator](phase3_lr_convergence_calc.html). Use ← → arrow keys.
...
| **3.5** | [phase3_5_cnn_mnist.html](phase3_5_cnn_mnist.html) | 13 slides (9 concepts + 4 hands-on exercises) | CNN vs FC + weight sharing, 2D convolution, Conv2D CUDA kernel code walkthrough, Max Pooling + index tracking, Full architecture, Training loop, Benchmarks, How to run, Summary. EXERCISES: Colab setup + conv by hand, Compile & train, Hyperparameter experiments, Nsight profiling + reflection |
| **3.5** | [🎬 Filter Sliding Animation](filter_sliding_animation.html) | Interactive | Watch a 3×3 filter slide across a 5×5 input, position by position. Computes all 9 multiplications at each step, builds the feature map. Play/Pause/Prev/Next controls + speed slider. |
...
| **6A** | [phase6A_tensorrt.html](phase6A_tensorrt.html) | 7 slides (5 concepts + 2 exercises) | 4 TRT optimizations, Export pipeline, FP32/FP16/INT8, Dynamic batching + serialization, Colab exercises with Phase 4 cross-references |
| **6B** | [phase6B_triton.html](phase6B_triton.html) | 8 slides (6 concepts + 2 exercises) | Model repository, config.pbtxt, HTTP/gRPC API, Dynamic batching, Multi-model GPU sharing, A/B testing, Docker+K8s deployment chain, Triton setup + inference exercises |
| **6C** | [phase6C_nsight.html](phase6C_nsight.html) | 6 slides (4 concepts + 1 exercise + summary) | Nsight Systems vs Compute, Roofline model (memory vs compute bound), Occupancy + warp stalls, Profiling workflow, conv2d_forward exercise |
| **6D** | [phase6D_streams.html](phase6D_streams.html) | 6 slides (3 concepts + 2 exercises + summary) | CUDA Streams, Default vs custom, Multi-stream 7.07× benchmark with real code, CUDA Graphs, Colab multistream demo, YOLO production pipeline exercise |
